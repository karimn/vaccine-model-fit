...
build_gmm_g <- function(candidate_data, replications, maxcand, use_corr_moments = FALSE, group_vaccines_by = vars(Platform, Subcategory), sim_seed = NULL, moments_to_use = everything()) {
...
gmm_g <- build_gmm_g(candidate_data, 10e3, 50, use_corr_moments = FALSE, group_vaccines_by = group_vaccines_by, sim_seed = 123, moments_to_use = moments_to_use)
...
test_results <- gmm(gmm_g, 
                    test_draws, 
                    # test_summaries, 
                    t0 = c(poverall=0.9, psubcat=0.9, pvector=0.8, psubunit=0.8, prna=0.6, pdna=0.4, pattenuated=0.8, pinactivated=0.8, ppreclinical=0.14, pphase1=0.23, pphase2=0.32, pphase3=0.5) %>% qlogis(), 
